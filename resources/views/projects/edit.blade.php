...
<div class="w-1/3 mx-auto mt-16">
    ...
    <form method="POST" action="{{$project->path()}}">
            ...
            <div>
                <div>
                    <textarea rows="3" class="textarea input @error('description') is-invalid @enderror" required placeholder="Project description" name="description">{{ $project->description }}</textarea>
                    @error('description')
                        @include ('error')
                    @enderror
                </div>
